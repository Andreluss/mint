...
struct mint
{
	...
	inline int _modulize(int x) const
	{
		x %= mod;
		if (x < 0) x += mod;
		return x;
	...
	mint operator+ (LL y) const
	{
		return *this + mint(y);
	}
	friend mint operator+ (int x, const mint& rhs)
	{
		return mint(x) + rhs;
	}
	friend mint operator+ (LL x, const mint& rhs)
	{
		return mint(x) + rhs;
	}
